...
class ArmorStandCmdManager {
    ...
    private void getCommandsFromScoreboardTags() {
        final Set<String> tags = new HashSet<>(armorStand.getScoreboardTags());
        for (String tag : tags) {
            if (tag.startsWith("ast-cmd-")) {
                final ArmorStandCmd command = ArmorStandCmd.fromLegacyTag(tag);
                armorStand.removeScoreboardTag(tag);
                if (command != null) {
                    addCommand(command, true);
                }
            } else if (tag.startsWith("ascmd::v2::")) {
                final ArmorStandCmd command = ArmorStandCmd.fromTag(tag);
                if (command != null) {
    ...
    void executeCommands(Player p) {
        ...
        for (int n = 0; n < commands.size(); n++) {
            final ArmorStandCmd asCmd = commands.get(n);
