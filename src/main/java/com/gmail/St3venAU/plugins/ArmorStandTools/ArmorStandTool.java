...
public enum ArmorStandTool {
    ...
    void showTitle(Player p) {
        ...
        final ChatColor offColor = ChatColor.WHITE;
        final ChatColor onColor = ChatColor.YELLOW;
        final ChatColor divColor = ChatColor.BLACK;
        final String msg =
    ...
    void use(Player p, ArmorStand as) {
        ...
        final boolean sneaking = reverseSneaking != p.isSneaking();
        final double yaw = getRelativeYaw(p, as);
    ...
    private double getRelativeYaw(Player p, ArmorStand as) {
        final double difference = p.getLocation().getYaw() - as.getLocation().getYaw();
    ...
    private String plrHeadName(ArmorStand as) {
        ...
        if (entityEquipment.getHelmet() == null || !(entityEquipment.getHelmet().getItemMeta() instanceof final SkullMeta meta))
            return null;
    ...
    static void updateTools(FileConfiguration config) {
        for (ArmorStandTool t : values()) {
            ...
            if (im != null) {
                ...
                final List<String> lore = config.getStringList("tool." + t.config_id + ".lore");
                if (t == GEN_CMD) {
                    final String cmdBlk = lore.size() > 0 ? lore.get(0) : "";
                    final String logged = lore.size() > 1 ? lore.get(1) : "";
                    lore.clear();
                    if (cmdBlk.length() > 0 && Config.saveToolCreatesCommandBlock)
                        lore.add(cmdBlk);
                    if (logged.length() > 0 && Config.logGeneratedSummonCommands)
                        lore.add(logged);
                }
                im.setLore(lore);
