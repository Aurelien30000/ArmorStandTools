...
public record ArmorStandCmd(String command, CommandType type, Integer priority,
    ...
    boolean execute(Player p) {
        if (command == null)
            return true;
        final String cmd = command.replaceAll("%player%", p.getName());
        switch (type) {
            case PLAYER -> {
    ...
    static ArmorStandCmd fromTag(String tag) {
        ...
        if (delay < 0)
            return null;
        final CommandType type = CommandType.fromTag(split[4]);
        if (type == null)
            return null;
        String cmd;
        if (split.length == 6) {
    ...
    static ArmorStandCmd fromLegacyTag(String tag) {
        ...
        if (!tag.startsWith("ast-cmd-"))
            return null;
        final CommandType type = CommandType.fromTag(tag.substring(8, 11));
        if (type == null)
            return null;
